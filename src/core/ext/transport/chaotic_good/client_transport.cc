  ...
  writer_ = MakeActivity(
      Loop(Seq(outgoing_frames_.Next(),
               [&control_endpoint, &data_endpoint](
                   FrameInterface* frame) -> LoopCtl<absl::Status> {
                 std::cout << "\n frame address " << frame;
                 fflush(stdout);
                 // auto client_frame =
                 // dynamic_cast<ClientFragmentFrame*>(frame); auto
                 // control_endpoint_buffer =
                 // client_frame->Serialize(&hpack_compressor); std::cout << "\n
                 // writer_ send next frame."; fflush(stdout); FrameHeader
                 // frame_header =
                 //     FrameHeader::Parse(
                 //         reinterpret_cast<const
                 //         uint8_t*>(grpc_slice_to_c_string(
                 ...
                 //     frame_header.message_padding; std::string
                 //     message_padding(message_padding_size, '0'); Slice
                 //     slice(grpc_slice_from_cpp_string(message_padding));
                 ...
                 //     std::cout << "data frame message length " <<
                 //     message_size; break;
                 //   }
